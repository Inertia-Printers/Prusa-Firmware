...
#define WRITE_LOGIC_ANALYZER_CH4(value) \
  if (value)                            \
    PORTK |= (1 << 0);                  \
  else                                  \
    PORTK &= ~(1 << 0)        // PK0
...
#define LOGIC_ANALYZER_CH4_ENABLE \
  do                              \
  {                               \
    DDRK |= 1 << 0;               \
  } while (0)
#define LOGIC_ANALYZER_CH5_ENABLE   \
  do                                \
  {                                 \
    cbi(UCSR2B, TXEN2);             \
    cbi(UCSR2B, RXEN2);             \
    cbi(UCSR2B, RXCIE2);            \
    SET_OUTPUT(LOGIC_ANALYZER_CH5); \
  } while (0)
#define LOGIC_ANALYZER_CH6_ENABLE   \
  do                                \
  {                                 \
    cbi(UCSR2B, TXEN2);             \
    cbi(UCSR2B, RXEN2);             \
    cbi(UCSR2B, RXCIE2);            \
    SET_OUTPUT(LOGIC_ANALYZER_CH6); \
  } while (0)
