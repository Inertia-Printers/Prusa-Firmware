...
void mmu_loop(void)
{
	...
	case S::Idle:
		...
		{
			//* Look for command from the ESP32
			if (READ(ESP_PIN))
			{
...
static void get_response_print_info(uint8_t move)
{
	...
	case MMU_NO_MOVE:
		...
		break;
	default:
		puts_P(PSTR("error: unknown move"));
		break;
	}
}
		...
		{ // no "ok" was received in reserved time frame, user will fix the issue on mmu unit
			...
			{ // screen 1
				if ((degTargetHotend(active_extruder) == 0) && turn_off_nozzle)
					lcd_display_message_fullscreen_P(_i("Press the knob to resume nozzle temperature.")); ////MSG_RESUME_NOZZLE_TEMP c=20 r=4
				else
					lcd_display_message_fullscreen_P(_i("Fix the issue and then press button on MMU unit.")); ////MSG_MMU_FIX_ISSUE c=20 r=4
				screen = 0;
			}
