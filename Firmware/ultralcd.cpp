...
#define MENU_ITEM_SDDIR(str_fn, str_fnl)    \
	do                                       \
	{                                        \
		if (menu_item_sddir(str_fn, str_fnl)) \
			return;                            \
	} while (0)
#define MENU_ITEM_SDFILE(str_fn, str_fnl)    \
	do                                        \
	{                                         \
		if (menu_item_sdfile(str_fn, str_fnl)) \
			return;                             \
	} while (0)
...
void lcd_commands()
{
	...
	if (lcd_commands_type == LcdCommands::Layer1Cal)
	{
		...
		if (lcd_commands_step > 1)
			lcd_timeoutToStatus.start(); // if user dont confirm live adjust Z value by pressing the knob, we are saving last value by timeout to status screen
		...
		if (!blocks_queued() && cmd_buffer_empty() && !saved_printing)
		{
			...
			case 2:
				...
				if (mmu_enabled)
					enquecommand_P(PSTR("M702 C")); // unload from nozzle
...
static void pgmtext_with_colon(const char *ipgmLabel, char *dst, uint8_t dstSize)
{
	uint8_t i = 0;
	for (; i < dstSize - 2; ++i)
	{ // 2 byte less than buffer, we'd be adding a ':' to the end
...
static void lcd_menu_debug()
{
	...
	lcd_printf_P(PSTR("RAM statistics\n"		////c=20
							...
							" heap_start: 0x%04x\n" ////c=14
							" heap_end: 0x%04x"),
					 SP_min, __malloc_heap_start, __malloc_heap_end); ////c=14
#endif																			  // DEBUG_STACK_MONITOR
...
static void lcd_menu_belt_status()
{
	...
	lcd_printf_P(PSTR("%S\n"
							" X %d\n"
							" Y %d"),
					 _T(MSG_BELT_STATUS), eeprom_read_word((uint16_t *)(EEPROM_BELTSTATUS_X)), eeprom_read_word((uint16_t *)(EEPROM_BELTSTATUS_Y)));
	menu_back_if_clicked();
}
#endif // TMC2130
...
void lcd_set_fan_check()
{
	...
	if (fans_check_enabled == false)
		fan_check_error = EFCE_OK; // reset error if fanCheck is disabled during error. Allows resuming print.
#endif									// FANCHECK
}
...
static void _lcd_move(const char *name, uint8_t axis, int min, int max)
{
	...
	if (lcd_encoder != 0)
	{
		...
		if (!planner_queue_full())
		{
			...
			if (min_software_endstops && current_position[axis] < min)
				current_position[axis] = min;
			if (max_software_endstops && current_position[axis] > max)
	...
	if (menu_leaving || LCD_CLICKED)
		(void)enable_endstops(_md->endstopsEnabledPrevious);
	if (LCD_CLICKED)
		menu_back();
}
...
static void lcd_menu_xyz_y_min()
{
	...
	for (uint8_t i = 0; i < 2; i++)
	{
		...
		if (distanceMin[i] >= 200)
			...
			lcd_printf_P(_N("%6.2fmm"), distanceMin[i]);
	}
	if (lcd_clicked())
...
static void lcd_babystep_z()
{
	...
	if (_md->status == 0)
	{
		...
		{
			_md->babystepMemZ = eeprom_read_word(reinterpret_cast<uint16_t *>(&(EEPROM_Sheets_base->s[(eeprom_read_byte(&(EEPROM_Sheets_base->active_sheet)))].z_offset)));
		}
	...
	if (lcd_encoder != 0)
	{
		...
		if (_md->babystepMemZ < Z_BABYSTEP_MIN)
			_md->babystepMemZ = Z_BABYSTEP_MIN; //-3999 -> -9.99 mm
		else if (_md->babystepMemZ > Z_BABYSTEP_MAX)
			_md->babystepMemZ = Z_BABYSTEP_MAX; // 0
		else
			babystepsTodoZadd(lcd_encoder);
...
void pid_extruder()
{
	...
	if (pid_temp > HEATER_0_MAXTEMP)
		pid_temp = HEATER_0_MAXTEMP;
	if (pid_temp < HEATER_0_MINTEMP)
		pid_temp = HEATER_0_MINTEMP;
	lcd_encoder = 0;
	lcd_set_cursor(1, 2);
...
bool lcd_wait_for_pinda(float temp)
{
	...
	while (current_temperature_pinda > temp)
	{
		...
		{ // PINDA cooling from 60 C to 35 C takes about 7 minutes
...
{
	if (SilentModeMenu_MMU == 1)
		SilentModeMenu_MMU = 0;
	else
...
static void crash_mode_switch()
{
	...
	if (IS_SD_PRINTING || usb_timer.running() || (lcd_commands_type == LcdCommands::Layer1Cal))
		menu_goto(lcd_tune_menu, 9, true, true);
	else
		menu_goto(lcd_settings_menu, 9, true, true);
}
#endif // TMC2130
...
void lcd_first_layer_calibration_reset()
{
	...
	if (LCD_CLICKED || !eeprom_is_sheet_initialized(eeprom_read_byte(&(EEPROM_Sheets_base->active_sheet))) ||
		 ...
		 (0 == static_cast<int16_t>(eeprom_read_word(reinterpret_cast<uint16_t *>(&EEPROM_Sheets_base->s[(eeprom_read_byte(&(EEPROM_Sheets_base->active_sheet)))].z_offset)))))
	{
		if (menuData->reset)
		{
...
#define SETTINGS_MMU_MODE                                                                   \
	...
	{                                                                                        \
		if (mmu_enabled)                                                                      \
		{                                                                                     \
			if (SilentModeMenu_MMU == 0)                                                       \
				MENU_ITEM_TOGGLE_P(_T(MSG_MMU_MODE), _T(MSG_NORMAL), lcd_silent_mode_mmu_set);  \
			else                                                                               \
				MENU_ITEM_TOGGLE_P(_T(MSG_MMU_MODE), _T(MSG_STEALTH), lcd_silent_mode_mmu_set); \
		}                                                                                     \
	} while (0)
...
#define SETTINGS_SD                                                                                                                \
	...
	{                                                                                                                               \
		...
		{                                                                                                                            \
		case SD_SORT_TIME:                                                                                                           \
			MENU_ITEM_TOGGLE_P(_T(MSG_SORT), _T(MSG_SORT_TIME), lcd_sort_type_set);                                                   \
			break;                                                                                                                    \
		case SD_SORT_ALPHA:                                                                                                          \
			MENU_ITEM_TOGGLE_P(_T(MSG_SORT), _T(MSG_SORT_ALPHA), lcd_sort_type_set);                                                  \
			break;                                                                                                                    \
		default:                                                                                                                     \
			MENU_ITEM_TOGGLE_P(_T(MSG_SORT), _T(MSG_NONE), lcd_sort_type_set);                                                        \
		}                                                                                                                            \
	} while (0)
...
{
	bool changed = false;
	if (tmc2130_wave_fac[X_AXIS] < TMC2130_WAVE_FAC1000_MIN)
		tmc2130_wave_fac[X_AXIS] = 0;
	if (tmc2130_wave_fac[Y_AXIS] < TMC2130_WAVE_FAC1000_MIN)
		tmc2130_wave_fac[Y_AXIS] = 0;
	if (tmc2130_wave_fac[Z_AXIS] < TMC2130_WAVE_FAC1000_MIN)
		tmc2130_wave_fac[Z_AXIS] = 0;
	if (tmc2130_wave_fac[E_AXIS] < TMC2130_WAVE_FAC1000_MIN)
...
uint8_t choose_menu_P(const char *header, const char *item, const char *last_item)
{
	...
	while (1)
	{
		...
		if (cursor_pos > 3)
		{
			...
			{ // here we are at the very end of the list
				Sound_MakeSound(e_SOUND_TYPE_BlindAlert);
			}
		}
		...
		if (last_item && last_visible)
			lcd_puts_at_P(1, 3, last_item);
...
static void lcd_main_menu()
{
	...
	if (!isPrintPaused && !IS_SD_PRINTING && !usb_timer.running() && (lcd_commands_type != LcdCommands::Layer1Cal))
	{
		if (!farm_mode)
		{
			...
			{ // show menu only if we have 2 or more sheets initialized
...
#define MENU_ITEM_EDIT_advance_K() \
	do                              \
	{                               \
		if (lcd_advance_K())         \
			return;                   \
...
void lcd_sdcard_menu()
{
	...
	case _standard: // normal menu structure.
	{
		...
		for (uint16_t i = _md->fileCnt; i-- > 0;) // Every file, from top to bottom.
		{
			if (menu_item == menu_line) // If the file is on the screen.
			{
				...
				MENU_ITEM_DUMMY(); // dummy item that just increments the internal menu counters.
		}
		MENU_END();
	}
	break;
	...
	break;
	default:
		_md->menuState = _uninitialized; // shouldn't ever happen. Anyways, initialize the menu.
	}
}
#ifdef TMC2130
static void lcd_belttest_v()
...
static void reset_crash_det(uint8_t axis)
{
	...
	plan_buffer_line_curposXYZE(manual_feedrate[0] / 60);
	st_synchronize();
	if (eeprom_read_byte((uint8_t *)EEPROM_CRASH_DET))
...
static bool lcd_selfcheck_axis_sg(uint8_t axis)
{
	...
	for (uint_least8_t i = 0; i < 2; i++)
	{ // check if measured axis length corresponds to expected length
	...
	{ // check if difference between first and second measurement is low
		// loose pulleys
		const char *_error_1;
...
static bool lcd_selfcheck_pulleys(int axis)
{
	...
	for (i = 0; i < 5; i++)
	{
		...
		st_synchronize();
		if (SilentModeMenu != SILENT_MODE_OFF)
			st_current_set(0, tmp_motor[0]); // set back to normal operation currents
		else
			st_current_set(0, tmp_motor_loud[0]); // set motor current back
		current_position[axis] = current_position[axis] - move;
...
static bool lcd_selfcheck_endstops()
{
		 ...
		 ((READ(Z_MIN_PIN) ^ Z_MIN_ENDSTOP_INVERTING) == 1))
	{
		...
		if ((READ(Y_MIN_PIN) ^ Y_MIN_ENDSTOP_INVERTING) == 1)
...
static bool lcd_selfcheck_check_heater(bool _isbed)
{
	...
	do
	{
		...
		if (_counter % 5 == 0)
			serialecho_temperatures(); // show temperatures once in two seconds
...
static bool selftest_irsensor()
{
	...
	{
	public:
		TempBackup() : m_temp(degTargetHotend(active_extruder)),
...
static FanCheck lcd_selftest_fan_auto(int _fan)
{
	...
	case 0:
		...
		if (fan_speed[0] < 20)
		{ // < 1200 RPM would mean either a faulty Noctua or Altfan
			return FanCheck::ExtruderFan;
		}
#ifdef FAN_SOFT_PWM
		else if (fan_speed[0] > 50)
		{ // printerFan is faster
...
static uint8_t lcd_selftest_screen(TestScreen screen, uint8_t _progress, uint8_t _progress_scale, bool _clear, uint16_t _delay)
{
	...
	if (screen == TestScreen::ExtruderFan)
		lcd_puts_P(_T(MSG_SELFTEST_FAN));
	if (screen == TestScreen::PrintFan)
		lcd_puts_P(_T(MSG_SELFTEST_FAN));
	if (screen == TestScreen::FansOk)
		lcd_puts_P(_T(MSG_SELFTEST_FAN));
	if (screen == TestScreen::EndStops)
		lcd_puts_P(_i("Checking endstops")); ////MSG_SELFTEST_CHECK_ENDSTOPS c=20
	if (screen == TestScreen::AxisX)
		lcd_puts_P(_T(MSG_CHECKING_X));
	if (screen == TestScreen::AxisY)
		lcd_puts_P(_T(MSG_CHECKING_Y));
	if (screen == TestScreen::AxisZ)
		lcd_puts_P(_i("Checking Z axis")); ////MSG_SELFTEST_CHECK_Z c=20
	if (screen == TestScreen::Bed)
		lcd_puts_P(_T(MSG_SELFTEST_CHECK_BED));
	if (screen == TestScreen::Hotend || screen == TestScreen::HotendOk)
		lcd_puts_P(_i("Checking hotend")); ////MSG_SELFTEST_CHECK_HOTEND c=20
	if (screen == TestScreen::Fsensor)
		lcd_puts_P(_T(MSG_SELFTEST_CHECK_FSENSOR));
	if (screen == TestScreen::FsensorOk)
		lcd_puts_P(_T(MSG_SELFTEST_CHECK_FSENSOR));
	if (screen == TestScreen::AllCorrect)
		lcd_puts_P(_i("All correct")); ////MSG_SELFTEST_CHECK_ALLCORRECT c=20
	if (screen == TestScreen::Failed)
		lcd_puts_P(_T(MSG_SELFTEST_FAILED));
	if (screen == TestScreen::Home)
		lcd_puts_P(_i("Calibrating home")); ////MSG_CALIBRATING_HOME c=20
	...
	else if (screen < TestScreen::Fsensor)
	{
		...
		lcd_selftest_screen_step(2, 2, ((screen == _step_block) ? 1 : (screen < _step_block) ? 0
																														 : 2),
										 PSTR("X"), _indicator);

		_step_block = TestScreen::AxisY;
		lcd_selftest_screen_step(2, 8, ((screen == _step_block) ? 1 : (screen < _step_block) ? 0
																														 : 2),
										 PSTR("Y"), _indicator);

		_step_block = TestScreen::AxisZ;
		lcd_selftest_screen_step(2, 14, ((screen == _step_block) ? 1 : (screen < _step_block) ? 0
																														  : 2),
										 PSTR("Z"), _indicator);

		_step_block = TestScreen::Bed;
		lcd_selftest_screen_step(3, 0, ((screen == _step_block) ? 1 : (screen < _step_block) ? 0
																														 : 2),
										 PSTR("Bed"), _indicator);

		_step_block = TestScreen::Hotend;
		lcd_selftest_screen_step(3, 9, ((screen == _step_block) ? 1 : (screen < _step_block) ? 0
																														 : 2),
										 PSTR("Hotend"), _indicator);
	}

	if (_delay > 0)
		delay_keep_alive(_delay);
	_progress++;
...
{
	{
		...
		if (0xff == autoDepleteRaw)
			lcd_autoDeplete = false;
		else
			lcd_autoDeplete = autoDepleteRaw;
	}
	backlight_init();
	lcd_init();
	lcd_refresh();
...
static void lcd_connect_printer()
{
	...
	while (no_response)
	{
		...
		if (i != 0)
			lcd_puts_at_P((i * 20) / (NC_BUTTON_LONG_PRESS * 10), 3, LCD_STR_SOLID_BLOCK[0]);
...
void lcd_ping()
{ // chceck if printer is connected to monitoring when in farm mode
	if (farm_mode)
	{
...
void menu_lcd_longpress_func(void)
{
	...
	{																						 // long press as live-adj-z
		if ((current_position[Z_AXIS] < Z_HEIGHT_HIDE_LIVE_ADJUST_MENU) // only allow live-adj-z up to 2mm of print height
			 && (menu_menu == lcd_status_screen									 // and in listed menus...
				  || menu_menu == lcd_main_menu || menu_menu == lcd_tune_menu || menu_menu == lcd_support_menu))
		{
			lcd_clear();
			menu_submenu(lcd_babystep_z);
		}
	...
	{ // long press as move-z
		if (menu_menu == lcd_status_screen || menu_menu == lcd_main_menu || menu_menu == lcd_preheat_menu || menu_menu == lcd_sdcard_menu || menu_menu == lcd_settings_menu || menu_menu == lcd_control_temperature_menu
#if (LANG_MODE != 0)
...
{
	return (menu_menu == lcd_babystep_z && lcd_timeoutToStatus.expired(LCD_TIMEOUT_TO_STATUS_BABYSTEP_Z));
}
static inline bool other_menu_expired()
{
	return (menu_menu != lcd_status_screen && menu_menu != lcd_babystep_z && lcd_timeoutToStatus.expired(LCD_TIMEOUT_TO_STATUS));
}
static inline bool forced_menu_expire()
{
	bool retval = (menu_menu != lcd_status_screen && forceMenuExpire);
...
bool UserECoolEnabled()
{
	...
	return (eeprom_read_byte((uint8_t *)EEPROM_ECOOL_ENABLE) == EEPROM_ECOOL_MAGIC_NUMBER) && (eeprom_read_byte((uint8_t *)EEPROM_EXPERIMENTAL_VISIBILITY) == 1);
}
